...
<xsl:stylesheet version="1.0"
        ...
        <xsl:choose>
            <xsl:when test="count(testsuite) = 0">
                ...
                <tr class="single-test">
                    ...
                    <td class="nr{$hasfailures}">
                        <xsl:value-of select="@failures"/>
                    </td>
                    <td class="nr{$haserrors}">
                        <xsl:value-of select="@errors"/>
                    </td>
                    <td class="time{$isslow}">
                        <xsl:value-of select="@time"/>
                    </td>
                </tr>
    ...
    <xsl:template match="//testsuite[count(testsuite) = 0]" mode="details">
        ...
        <xsl:choose>
            <xsl:when
                test="count(testcase) &gt; 0 and ( $show_success_detail = 1 or ( $show_success_detail = 0 and ( @failures &gt; 0 or @errors &gt; 0 ) ) )">
    ...
    <xsl:template match="error">
        <tr>
            <td class="error-detail-type" colspan="3">
                <xsl:value-of select="@type"/>
            </td>
        </tr>
        <tr>
            <td class="error-detail-detail" colspan="3">
                <xsl:value-of select="."/>
            </td>
        </tr>
    </xsl:template>
    ...
    <xsl:template match="failure">
        <tr>
            <td class="fail-detail-type" colspan="3">
                <xsl:value-of select="@type"/>
            </td>
        </tr>
        <tr>
            <td class="fail-detail-detail" colspan="3">
                <xsl:value-of select="."/>
            </td>
        </tr>
    </xsl:template>
